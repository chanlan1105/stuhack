...
function addButtons() {
	...
	button1.classList.add("unlocked-download-button");
	...
	// Find download buttons that are paywalled
	const paywalledBtnIcons = document.getElementsByClassName("fa-cloud-arrow-down");

	for (let i = 0; i < paywalledBtnIcons.length; i++) {
		const paywaleldBtn = paywalledBtnIcons[i].parentNode.parentNode;
		if (paywaleldBtn.classList.contains("unlocked-download-button")) {
			// Already unlocked, skip
			continue;
		}

		// Remove paywalled button and replace with a new one
		const parent = paywaleldBtn.parentNode;
		paywaleldBtn.remove();

		const unlockedBtn = button1.cloneNode(true);
		unlockedBtn.onclick = downloadDoc;

		parent.prepend(unlockedBtn);
	}
}
